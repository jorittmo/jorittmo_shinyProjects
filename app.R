                ...
                tabPanel("UN Voting",
                         sidebarLayout(
                           sidebarPanel(h3("UN Voting"),
                                        p("Cluster countries and explore voting patterns for the United Nation general assembly. Choose time range and/or specific topics to explore."),
                                        useShinyjs(),
                                        sliderInput("year2",
                                                    "Year:",
...
server <- function(input, output, session) {
  ...
  observeEvent(c(input$year2, input$na_prop, input$topic, input$clust, input$gdp_filt), {
